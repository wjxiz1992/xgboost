...
struct ElementLoader {
  ...
  __device__ ElementLoader(bool use_shared, const size_t* row_ptr,
                           const Entry* entry, int num_features,
...
__global__ void PredictKernel(const DevicePredictionNode* d_nodes,
                              ...
                              const Entry* d_data, size_t tree_begin,
...
class GPUPredictor : public xgboost::Predictor {
 ...
 private:
  void DevicePredictInternal(DMatrix* dmat,
    ...
    // TODO: make it a class member if it affects performance
    thrust::device_vector<bst_float> predictions(out_preds->Size());
    ...
    devices = GPUSet::Range(param.gpu_id, param.gpu_id + 1);    
    batch.offset.Reshard(GPUDistribution::Overlap(devices, 1));
    std::vector<size_t> batch_offsets{0, batch.data.Size()};
    batch.data.Reshard(GPUDistribution::Explicit(devices, batch_offsets));
    
    const int BLOCK_THREADS = 128;
    const int GRID_SIZE = static_cast<int>(
        dh::DivRoundUp(batch.offset.Size() - 1, BLOCK_THREADS));

    int shared_memory_bytes = static_cast<int>(
        sizeof(float) * dmat->Info().num_col_ * BLOCK_THREADS);
    ...
    PredictKernel<BLOCK_THREADS><<<GRID_SIZE, BLOCK_THREADS, shared_memory_bytes>>>
      (dh::Raw(nodes), predictions.data().get(),
       dh::Raw(tree_segments), dh::Raw(tree_group),
       batch.offset.DevicePointer(param.gpu_id),
       batch.data.DevicePointer(param.gpu_id),
       tree_begin, tree_end, dmat->Info().num_col_,
       dmat->Info().num_row_, use_shared, model.param.num_output_group);
